...
import { Form, Input, Button, InputNumber, DatePicker, Select, Spin, Icon, Tooltip } from 'antd';
import { transformCommaString } from '../../constants/util'
	...
	handleSubmit(e) {
		...
		this.props.form.validateFieldsAndScroll((err, values) => {
			if (!err) {
				...
				if (values.order_id !== "") {
					values.order_id = transformCommaString(values.order_id)
				}
				values.page = 1;
				values.page_size = 20;
			...
			<fieldset className="margin-top-md form_container filter-form-box">
				...
				{/*input输入框搜索*/}
				{
					[
						{
							label: <span>
								<Tooltip placement="bottomLeft" title="多订单筛选可用空格或中英文逗号隔开">
									<Icon style={{ marginRight: '3px' }} type="question-circle" theme="outlined" />
									<span>订单ID</span>
								</Tooltip>
							</span>,
							name: 'order_id'
				...
				<FormItem label="公司简称" key="company_name">
