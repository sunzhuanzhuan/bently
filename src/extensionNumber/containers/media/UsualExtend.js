		...
		let columns = [
			{
				...
				render: (created_time, { first_online_time }) => {
					return <div style={{ textAlign: 'left' }}>
						<span>入库：{created_time ? moment(created_time * 1000).format('YYYY-MM-DD') : '-'}</span><br />
				...
				width: 128,
				render: (text) => {
					return <span>{text ? moment(parseInt(text) * 1000).format('YYYY-MM-DD') : '-'}</span>
				}
			}, {
				title: '播放量/阅读量',
				dataIndex: '3',
					...
					return <div>
						{
							item.status == 1 ?
								...
								item.status == 2 ?
									<Button type="primary" size='small' onClick={() => {
										window.open(window.location.origin + '/account/manage/update/' + item.weibo_type + '?account_id=' + item.account_id)
									}}>下架</Button> :
									item.status == 3 ?
										<Button type="primary" size='small' onClick={() => {
											window.open(window.location.origin + '/account/manage/update/' + item.weibo_type + '?account_id=' + item.account_id)
...
class SearchInput extends React.Component {
	...
	render() {
		...
		const { weibo_type, babysitter_host, operationUrl } = this.props;
		return (<div>
			选择主账号：
            <Select
				className='mr10'
			...
			<Button type='primary'
				...
				onClick={() => {
					window.open(window.location.origin + '/account/manage/add/' + weibo_type + '?user_id=' + value.key + '&fetch_info=' + operationUrl.slice(operationUrl.indexOf("=") + 1, operationUrl.length));
				}}>去入库</Button>
