...
import WechatEditableTable from '../../components/WechatEditableTable'
...
class ImportNumber extends Component {
	...
	submitNumber = () => {
		...
		// 处理成功的数据
		let body = this.state.platformType == 9 ?
			this.state.successAry.map(({ n, m }) => ({
				account_name: n,
				url: m,
	...
	retweet = (fun) => {
		this.setState({
			errorAry: [...this.state.errorAry.map(item => {
				return { ...item, n: "" }
			})]
		}, fun)
	}
						...
						{errorAry.length > 0 ?
							platformType == 9 ?
								<div>
									...
									<span>微信仅支持【账号名】【历史图文链接】导入，请核对各项正确性</span>
									<WechatEditableTable
...
class WrappedForm extends Component {
	...
	render() {
		...
		return (<Form>
			...
			{
				item.name == "微信" ?
					<FormItem label={"请将您表格中的" + ((item && item.text)) + "复制到文本框"}>
