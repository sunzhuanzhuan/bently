...
import { Table, Input, Popconfirm, Divider, message, Icon } from 'antd';
import { platformTypesMap } from '../constants/config';

import './WechatEditableTable.less'
message.config({
	top: 20
})
...
class WechatEditableTable extends Component {
	constructor(props) {
		super(props);
		let titleM = props.typeMap || '链接'
		this.columns = [
			{
				title: <div><span>账号名</span>
					</div>,
				dataIndex: 'n',
				width: '25%',
		...
		this.props.retweet(
			() => {
				...
				this.props.data.forEach(target => {
					let flag = (k1reg.test(target.n) && k2reg.test(target.m))
					if (flag) {
						this.onDelete(target.key)
						setSuccessAry({ ...target, n: target.m, v: flag })
					}
					return
				})
			}
	...
	async save(key) {
		...
		if (target) {
			...
			if (flag) {
				this.onDelete(key)
				setSuccessAry({ ...target, n: target.n,m:target.m, v: flag })
				return
			}
			this.error()
...
export default WechatEditableTable
