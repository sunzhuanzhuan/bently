        ...
        return (
            <div className='filter-common'>
                <div className="filter-common-items">
                    {
                        mapFieldsToPlatform['kolVisitorGenderDrawType'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='受众性别' className='dropdown-menu'>
                    ...
                    {
                        mapFieldsToPlatform['kolVisitorAgeDraw'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='受众年龄' className='dropdown-menu'>
                            {getFieldDecorator("kolVisitorAgeDraw", {})(
                                <SelectAndInput
                                    promptMessage="可以选择占比大于某个值的年龄段"
                                    options={kolVisitorAgeDrawOptions}
                                    onOkClick={(values) => {onChange('kolVisitorAgeDraw', '受众年龄', values)}}
                                    selectedItems={this.props.selectedItems}>
                                </SelectAndInput>
                            )}
                        </DropdownMenuNew>
                    ...
                    {
                        mapFieldsToPlatform['kolVisitorProvinceDraw'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='受众地域' className='dropdown-menu'>
                            {getFieldDecorator("kolVisitorProvinceDraw", {})(
                                <SelectAndInput promptMessage="可以选择占比大于某个值的地域" options={kolProvinceList} onOkClick={(values) => {
                                    onChange('kolVisitorProvinceDraw', '受众地域', values)
                                }} selectedItems={this.props.selectedItems}></SelectAndInput>
                            )}
                        </DropdownMenuNew>
                    }
                    {
                        mapFieldsToPlatform['kolVisitorInterestDraw'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='受众兴趣' className='dropdown-menu'>
                            {getFieldDecorator("kolVisitorInterestDraw", {})(
                                <SelectAndInput promptMessage="可以选择占比大于某个值的兴趣" options={kolInterestList} onOkClick={(values) => {
                                    onChange('kolVisitorInterestDraw', '受众兴趣', values)
                                }} selectedItems={this.props.selectedItems}></SelectAndInput>
                            )}
                        </DropdownMenuNew>
                    }
                    {/* 微信公众号 */}
                    {
                        mapFieldsToPlatform['trueReadRatio'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='真实阅读率' className='dropdown-menu'>
                            {getFieldDecorator("trueReadRatio", {})(
                                <SelectAndInput id='trueReadRatio' onOkClick={(values) => {
                                    onChange('trueReadRatio', '真实阅读率', values)
                                }} selectedItems={this.props.selectedItems}></SelectAndInput>
                            )}</DropdownMenuNew>
                    }
                    {
                        mapFieldsToPlatform['mediaIndexOneAvgReadNum'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='头条阅读量' className='dropdown-menu'>
                            {getFieldDecorator("mediaIndexOneAvgReadNum", {
                                ...
                                <IndexButton id='mediaIndexOneAvgReadNum' onOkClick={(values) => {
                                    onChange('mediaIndexOneAvgReadNum', '头条阅读量', values)
                                }} inputLableAfter=""></IndexButton>
                            )}</DropdownMenuNew>
                    }
                    {/* 新浪微博 */}
                    {
                        mapFieldsToPlatform['directMediaInteractionAvg'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='直发互动量' className='dropdown-menu'>
                            {getFieldDecorator("directMediaInteractionAvg", {
                                ...
                                <IndexButton promptMessage="直发类型微博的平均转发、评论、点赞之和" id='directMediaInteractionAvg' onOkClick={(values) => {
                                    onChange('directMediaInteractionAvg', '直发互动量', values)
                                }} inputLableAfter=""></IndexButton>
                            )}
                        </DropdownMenuNew>
                    }
                    {
                        mapFieldsToPlatform['trueFansRate'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='真粉率' className='dropdown-menu'>
                            {getFieldDecorator("trueFansRate", {})(
                                <SelectAndInput id='trueFansRate' onOkClick={(values) => {
                                    onChange('trueFansRate', '真粉率', values)
                                }} selectedItems={this.props.selectedItems}></SelectAndInput>
                            )}
                        </DropdownMenuNew>
                    }
                    ...
                    {
                        mapFieldsToPlatform['areaIds'].includes(platformType) &&
                        <DropdownMenuNew visible={dropdownMenuShow} name='账号地域' className='dropdown-menu'>
                            {getFieldDecorator("areaIds", {
                                // initialValue: '0'
                            })(
                                <TreeTransfer options={defaultHotCities} onOkClick={(values) => {
                                    onChange('areaIds', '账号地域', values)
                                }} onClickCancel={this.onClickCancel}></TreeTransfer>
                            )}
                        </DropdownMenuNew>
                    }
                    ...
                    {/* 小红书,其他平台 */}
                    {
                        ...
                        <DropdownMenuNew visible={dropdownMenuShow} name='账号性别' className='dropdown-menu'>
                            {getFieldDecorator("gender", {
                                // initialValue: '0'
                            })(
                                <SelectMenu
                                    onSelect={(values) => {
                                        onChange('gender', '账号性别', values)
                                    }}
                    ...
                    }
                    {
                        ...
                        <DropdownMenuNew visible={dropdownMenuShow} name='账号行业' className='dropdown-menu'>
                            {getFieldDecorator("industryId", {
                                // initialValue: '0'
                            })(
                                <SelectMenu
                                    onSelect={(values) => {
                                        onChange('industryId', '账号行业', values)
                                    }}
                    ...
                    }
                    {
                        ...
                        <DropdownMenuNew visible={dropdownMenuShow} name='认证类型' className='dropdown-menu'>
                            {getFieldDecorator("verifiedStatus", {
                                // initialValue: '0'
                            })(
                                <SelectMenu
                                    onSelect={(values) => {
                                        onChange('verifiedStatus', '认证类型', values)
                                    }}
                    ...
                    {mapFieldsToPlatform['readPrice'].includes(platformType) &&
                    <DropdownMenuNew visible={dropdownMenuShow} name='阅读单价' className='dropdown-menu'>
                        <div className='' style={{'overflow': 'hidden', marginBottom: '20px'}}>
                            {getFieldDecorator('skuUnitReadPrice', {})(
                                <SelectAndInput
                                    onOkClick={(values) => {
                                        onChange('skuUnitReadPrice', '阅读单价', values)
                                    }}
                    ...
                    {mapFieldsToPlatform['playPrice'].includes(platformType) &&
                    <DropdownMenuNew visible={dropdownMenuShow} name='播放单价' className='dropdown-menu'>
                        <div className='' style={{'overflow': 'hidden', marginBottom: '20px'}}>
                            {getFieldDecorator('skuUnitPlayPrice', {})(
                                <SelectAndInput
                                    onOkClick={(values) => {
                                        onChange('skuUnitPlayPrice', '播放单价', values)
                                    }}
