			...
			<div className="account-list-main-item-content" style={{ clear: "both" }}>
				...
				<footer className="content-footer">
					...
					<div className='footer-info-status'>
						{/* 此处为右下角展示统计信息 */}
						{IsOther ? null : <InfoDIsplay title={<span>SNBT值 <MarkMessage {...messageInfo['snbt']} /></span>} value={snbt} format="0.0" isString={true} />}
						{IsWeibo ? <InfoDIsplay title={<span>真粉率<MarkMessage {...messageInfo['zhenfen']} /></span>} value={trueFansRate} format="0.0%" isString={true} /> : null}
						{ISWEiXin ? <InfoDIsplay title={<span>真实阅读率<MarkMessage {...messageInfo['yudu']} /></span>} value={trueReadRatio} format="0.0" afterLable="%" isString={true} /> : null}
