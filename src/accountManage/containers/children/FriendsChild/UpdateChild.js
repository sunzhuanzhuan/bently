	...
	render() {
		...
		return (Object.keys(priceInfo).length) ?
			<Tabs defaultActiveKey={addQuote ? '2' : '1'} animated={false} tabBarExtraContent={
				...
				<TabPane tab="帐号信息" key="1">
					<AccountInfos params={params}>
						...
						<WrapPanelForm onSave={this.saveFans} navId='fansInfos' header='粉丝信息' action={updateAccountInfo}>
							<Fans {...params}
								isFansNumberImg={pid == platformMap['PENG_YOU_QUAN']}
				...
				<TabPane key="2" tab={
					<span>报价信息{((is_famous == 1) && hasRedDot(price_valid_to)) ? <Badge dot><b style={{ visibility: 'hidden' }}>_</b></Badge> : null}</span>}>
					{!this.state.isLoading ? <AccountPriceForm params={params} getSkuActions={this.getSkuActions} /> : <Skeleton active />}
				</TabPane>
...
export class BaseInfoForm extends Component {
	...
	render() {
		...
		const { platform_name, platform_icon, platform_id } = accountInfo
		...
		return <Form>
			<WrapPanel header='基础信息' navId='baseInfos' left={baseInfoLeft} right={baseInfoRight}>
				<BaseInfo {...params} {...form} hideUniqueId={true} hideLink={platform_id == 23}>
					{platform_id == platformMap['PENG_YOU_QUAN'] ? <QCCodeUpload  {...params} {...form} /> : <i style={{ display: 'none' }} />}
...
export class AccountPriceForm extends Component {
	handlePrice = (price_item_list, price_now = {}, price_next = {}) => {
		console.log(price_now, price_next);
...
export class AccountFeatureForm extends Component {
	...
	onSave(values) {
		// console.log(this.webpackExtraFormData(values), 'webpackExtraFormDatawebpackExtraFormDatawebpackExtraFormData')
