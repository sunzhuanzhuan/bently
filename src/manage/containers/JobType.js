...
// import * as actionAuth from '../../auth/actions/roles'
import {
	Input, Table, Button, Modal, Form, message, Popover, Checkbox,Select
} from "antd";
import './branch.less'
import qs from "qs";
...
class Branch extends Component {
	...
	getList = async (query = { page: 1, page_size: 20 }) => {
		...
		getRoleJobList({ app_id: 1, page: 1 })
	...
	render() {
		...
		let { jobTypeList, branchListAll, addJobType, queryJobTypeSucc, editJobType, branchTree, getBranchTree, jobRoleListAll ,getRoleJobList} = this.props;
		...
		return (
			<div className='branch'>
				...
				<Select
					...
					placeholder="请选择应用"
					onChange={this.handleGetApp.bind(this)}
					defaultValue={1}
				>
					{this.state.applist.map(d => <Option value={d.app_id} key={d.app_id}>{d.app_name}</Option>)}
				</Select>
					<Search
						style={{ width: '200px' }}
						className='search'
...
class BranchForm extends Component {
	...
	handleSubmit = (e) => {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				this.props.getRoleJobList({ app_id: 1, page: 1 })
			}
		});
	}
	...
	handleAppChange=(value)=>{
		this.props.getRoleJobList({ app_id: value, page: 1 })
	}
	componentWillMount() {
		api.get('/rbac/getAppInfo').then((response) => {
			this.setState({
